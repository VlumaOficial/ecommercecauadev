...
**📌 Feature "notificar quando o vendedor modifica o pedido" — Incremento (A): previsão de entrega no `pedido_validado` — implementado 02/09/2026 (insert-only).** O texto do `pedido_validado` (email + whatsapp) passa a incluir "Previsão de entrega: {data_prevista}." (migration `048`, aplicada pelo PO). **Decisão de produto: a regra de fallback do placeholder `{data_prevista}` vive na RESOLUÇÃO (orquestrador `notificar-pedido.ts`), não no texto do template** — `data_prevista` preenchida resolve para `dd/mm/aaaa`, vazia/null resolve para o literal **`"a combinar"`**. Motivo: o placeholder tem que resolver sempre para algo válido mesmo que o lojista edite o texto do template no futuro (visão SaaS — templates viram editáveis por-tenant). A chave `data_prevista` fica sempre presente no `vars` (como `motivo`), disponível para qualquer template. Nenhum outro evento muda; sem tela. **Incremento (B) — remarcação de `data_prevista` depois da validação — DESENHO + migrations criadas 02/09/2026 (não aplicadas), aguardando avaliação do PO antes do código.** Regras de negócio fixadas pelo PO: remarcar só em `aguardando_validacao`/`confirmado`; diálogo "Remarcar entrega" (nova data + motivo); evento **específico `pedido_data_remarcada`** (texto ao cliente mostra a nova data via `{data_prevista}`); **motivo obrigatório e interno** — o cliente NUNCA vê o motivo da remarcação (mesma classe de `observacao_interna`); toda remarcação bem-sucedida grava uma linha imutável em `order_delivery_reschedules` (histórico/rastreabilidade para relatórios futuros — visão SaaS) e atualiza `orders.data_prevista`, na mesma transação (RPC `remarcar_entrega_pedido`). Decisão: notifica-se **sempre** numa remarcação bem-sucedida, inclusive quando não havia previsão anterior (ir de "a combinar" para uma data é uma atualização). Migrations `049`/`050`/`051`, decisões (a)/(b) e o desenho completo em `ESCOPO_PROJETO.md` §0 item 52. Detalhe do levantamento e do incremento (A) idem.
