...
**Achado durante a implementação, migration nova criada (`040`, ainda não aplicada)**: a `039` não trouxe nenhuma RPC de escrita pra `observacao_interna` (só a coluna, só leitura pro staff) — o campo "Observação interna", editável pelo vendedor, precisa da RPC `atualizar_observacao_interna_pedido` (migration `040`) pra persistir. Até essa migration ser aplicada, o resto da tela funciona normalmente, mas salvar uma observação interna mostra um erro (confirmado com Chromium real — comportamento esperado, não é bug do restante da tela).
