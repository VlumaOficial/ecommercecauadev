...
8. **Migration `020` (drop da coluna antiga `products.unidade_venda`) ainda não aplicada** — 1ª tentativa falhou (`products_com_status` depende da coluna via `p.*`), arquivo já corrigido (`DROP VIEW` + `DROP COLUMN` + `CREATE VIEW` recriando com `security_invoker = true`), aguardando nova tentativa do usuário. Ver `docs/MIGRATIONS.md` (migration `020`).
9. **Tela de gestão `/painel/unidades-venda` ainda não implementada** — o cadastro de produto já funciona (Combobox lê as unidades via `/api/painel/unidades-venda`, Route Handlers já existem), mas falta a tela do painel pra o lojista criar/editar/inativar unidades além das 6 do seed inicial. Fora do caminho crítico (não bloqueia cadastro de produto).
