...
import { criarCategoriaComoStaff } from '@/lib/painel/categorias'
...
export async function POST(request: NextRequest) {
  ...
  const supabase = await createClient()
  const resultado = await criarCategoriaComoStaff(
    supabase,
    {
  ...
  return NextResponse.json({ data: resultado.categoria }, { status: 201 })
}
